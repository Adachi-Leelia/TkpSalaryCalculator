...
- 主要な日々の訪問入力と訪問内タスクの編集は1画面内で完了させる。
...
| `SCR-ANNUAL-01` | 年間累計設定 | 設定 | 設定メニュー |
...
- 割増または件数加算を無効化すると既存勤務の金額が変わる場合は、重複を除いた影響訪問数と変更前後の見込み差額を保存前に表示する。
- 単価行の削除などによって対象年月の勤務記録が未計算になる場合は、重複を除いた影響訪問数と理由を保存前に表示する。
...
- 保存済み年間締め月による年間区分の開始給与期間から選択中給与期間までの「年間給与見込み累計」
- 年間累計の給与期間年月範囲（例：`2026年1月分～8月分`）
- 年間範囲内に未計算勤務がある場合だけ、`未計算N件を除く`
- 未計算の訪問数
...
月次の給与見込み合計を主表示として維持する。年間累計は同じ給与見込み合計カード内で月次内訳の後に区切って表示し、月次合計より小さいが判読可能な文字サイズとする。年間累計が0円でも表示し、未計算件数が0件なら年間の注意文を表示しない。前給与期間、次給与期間または現在の給与期間へ移動した場合、および勤務記録、給与設定、締め日履歴または月額手当の変更後は、月次と年間の表示を同じ要求結果で更新する。遅れて完了した古い要求で新しい選択期間を上書きしない。
...
- 選択日、曜日および日別給与見込み合計
- 訪問の先頭数件と総訪問数。各訪問は「身体1、生活3」のようにタスクを要約し、全件を開く操作で`SCR-DAY-01`へ遷移
...
- 対象日と曜日
- 日別給与見込み合計
- 訪問ごとのタスク要約、タスク数、訪問合計、計算状態
- 基本シフトからの反映候補

操作：

- 新規訪問を追加
- 訪問とその全タスクを一括して編集または削除
- 訪問の計算内訳を表示
- 基本シフトを反映
- 別の日の勤務記録をこの日へ複製

対象曜日に有効な基本シフトがある場合は、カレンダーの日付選択後に反映候補を作成する。ただし、自動保存も確認画面の自動表示もせず、利用者が反映候補を開いた場合だけ`DLG-SHIFT-01`で確定を求める。同じ基本シフト由来の訪問またはタスク多重集合が類似する既存訪問がある場合は、重複候補として表示する。

### 7.4 `SCR-WORK-01` 勤務入力・編集

画面上部に訪問全体で共通の勤務日を表示し、その下に1件以上のタスクカードを表示する。新規訪問は空のタスク1件で開始する。各タスクカードの入力項目は次のとおりとする。

| 項目 | 入力単位 | 必須条件 |
| --- | --- | --- |
| 勤務日 | 訪問 | 常に必須。全タスクで共通 |
| サービス設定 | タスク | 登録済みの組み合わせを利用する場合 |
| サービス種類 | タスク | 常に必須。サービス設定選択時は自動入力 |
| 時間区分 | タスク | 時間区分を利用する場合 |
| 入力方式 | タスク | `開始・終了時刻`または`勤務時間` |
| 開始時刻・終了時刻 | タスク | `開始・終了時刻`方式の場合。勤務時間は自動算出 |
| 勤務時間 | タスク | 勤務時間入力方式の場合 |
| 開始時刻 | タスク | 勤務時間入力方式で、選択日・サービスに時刻条件付き割増が適用可能な場合。終了時刻は自動算出 |

挙動：

- `タスクを追加`で空のタスクカードを末尾へ追加し、その時点でタスクIDを採番する。
- 各タスクを編集、上へ移動、下へ移動および削除できる。並べ替え後の表示順は0始まりの連番へ正規化する。
- 最後の1タスクでは削除操作を無効化し、理由をアクセシビリティラベルでも説明する。
- 同じサービス種類・時間区分のタスクを複数選択しても警告または禁止を行わない。
- サービス設定を選択すると、そのタスクのサービス種類と標準時間を自動入力する。
...
- 新規追加した各タスクでは、サービス設定、サービス種類、時間区分および勤務時間を未選択・未入力で表示する。直前に確定したタスクの値や候補一覧の先頭を自動選択しない。
- サービス設定は、入力候補に設定された表示順の昇順で画面上部の候補へ表示し、選択後も保存前に編集できる。直近の使用または使用回数によって候補を並べ替えない。同じ表示順の場合はサービス設定名の昇順とする。
- 各タスクカードにタスク給与のプレビューを表示し、その後に訪問の件数加算と訪問合計を表示する。1件でも計算できないタスクがある場合は訪問合計を表示せず、タスク番号と不足理由を表示する。
- 訪問内の合計勤務時間やタスク間の重複・空き時間は表示上の警告条件にしない。
- 保存時は訪問と全タスクを一括送信する。最初の不正タスクへフォーカスし、保存途中に失敗した場合も一部のタスクだけを確定しない。

編集画面では、勤務日を別の年月へ変更すると全タスクの適用設定も変わることを保存前に表示する。未保存変更の判定は、タスクの追加、編集、並べ替えおよび削除を含む。
...
- 各タスクへ適用された基本単価と単価方式
- 各タスクの基本給与
- 各タスクへ適用された割増の表示名、条件、計算対象時間および金額
- 各タスクの小計
- 訪問へ適用された件数加算の表示名と金額
- 訪問合計
- 使用した設定対象年月

未計算訪問では、該当タスクごとの不足理由を表示し、件数加算と訪問合計を推測表示しない。給与期間単位では、全タスクから集計した日別小計、基本給与合計、割増種別ごとの合計、各訪問から集計した件数加算合計、月額手当および期間合計を表示する。
...
| グループ | 項目 |
| --- | --- |
| 給与設定 | サービス・単価、割増、件数加算、月額手当、給与期間、年間累計 |
...
初期表示は現在年月とし、対象年月だけを変更することを常時表示する。新しく使用する年月は直近月の設定を自動で引き継ぐ。既存の対象年月を直近月と同じ内容へ戻す`前月の設定をコピー`操作を提供し、保存前に重複を除いた影響訪問数と見込み差額を表示する。
...
### 8.8 `SCR-ANNUAL-01` 年間累計設定

入力項目は「年間締め月」のPickerとし、1月から12月を表示する。既定値は12月とする。

選択値に応じて年間区分の例を表示する。12月締めは「1月分～12月分」、3月締めは「前年4月分～当年3月分」とする。「この設定は年間累計の区切りだけを変更し、給与額や勤務記録は変更しません」と説明する。

固定保存バーを使用し、未保存のまま戻る場合は破棄確認を表示する。保存成功後は設定メニューへ戻って成功メッセージを表示し、次のホーム表示で新しい年間区分を再計算する。

### 8.9 `SCR-SHIFT-01`／`SCR-SHIFT-02` 基本シフト

曜日ごとにグループ化し、同じ曜日内で複数の基本シフトを並べ替えられるようにする。各基本シフトは1件以上のタスクを持ち、編集画面では`SCR-WORK-01`と同じタスクの追加、編集、並べ替え、削除および最後の1タスクを削除できない規則を使用する。基本シフト自体の表示順と有効状態も編集できる。

設定不備の基本シフトには、該当タスクと理由を示す警告を表示し、勤務記録へ黙って反映しない。
...
複製先では元の各訪問とその全タスクを独立した新規記録として一括保存する。

### 10.3 `DLG-IMPORT-01` インポート確認

- エクスポート作成日時
- データ形式バージョン
- 設定年月、基本シフト数、基本シフトタスク数、訪問数、訪問タスク数および月額手当の件数。形式1・2では変換後のタスク数が親数と同じであることを表示
- 現在のデータをすべて置き換える旨
- 取消と実行
...
- `SCR-HOME-01`の年間給与見込み累計は、集計範囲、金額および未計算件数を1つの説明として読み上げる。
- 訪問の要約は「訪問1件、タスク2件、身体1、生活3、件数加算150円」のように、訪問数とタスク数を区別して読み上げる。
- タスクカードの読み上げ順とキーボードフォーカス順は表示順に一致させ、追加・移動・削除後も現在のタスクをタスクIDで維持する。
