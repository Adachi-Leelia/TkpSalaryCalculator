...
よく使うサービス設定名、サービス種類、標準時間、選択中年月の単価方式、単価および有効状態を一覧表示する。並べ替え、追加、編集および無効化を提供する。
...
- 選択中年月の給与設定：サービス種類、時間区分名と標準時間、単価方式、基本単価、対象年月での有効状態

サービス種類または時間区分そのものを追加する操作と、既存の組み合わせを入力候補として登録する操作を区別する。全期間共通の入力候補を変更する場合は、作成済み勤務記録と過去年月の給与計算は変わらないことを保存前に表示する。
