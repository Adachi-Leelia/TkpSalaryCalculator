...
**完了条件**: 開始日・終了日を常に明記し、期間移動で月次の小計・合計・未計算件数と、年間の範囲・累計・未計算件数が同時に更新される。年間累計は0円でも表示し、範囲・金額・未計算件数をアクセシビリティ説明として提供する。
...
**状態**: 1回目レビュー完了、初回レビュー指摘事項修正済み、再レビューまだ

**TODO**: 再レビューおよびその修正（一通り実装が完了してから行う。指示があるまで対応しない）
